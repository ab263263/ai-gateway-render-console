...
pub async fn check_all_platforms(state: Arc<ProxyState>) {
    ...
    let platforms = match web::block(move || crate::db::platform::list(&db)).await {
        ...
        Err(e) => {
            tracing::error!("health check: failed to join blocking platform list: {}", e);
            return;
        }
    };
    ...
    for platform in platforms {
        ...
        check_single_platform(state.db.clone(), http_client.clone(), platform_id, base_url, api_key).await;
    }
}
...
async fn check_single_platform(db: DbPool, http_client: Client, platform_id: String, base_url: String, api_key: String) {
    ...
    match result {
        Ok(resp) if resp.status().is_success() => {
            ...
            web::block(move || {
                crate::db::platform::record_health_success(&db_clone, &platform_id_clone, &now_clone, latency_ms)
            }).await.ok();
        }
        Ok(resp) => {
            tracing::warn!("health check FAIL: platform_id={} status={} latency={}ms", platform_id, resp.status().as_u16(), latency_ms);
            let db_clone = db.clone();
            let platform_id_clone = platform_id.clone();
            let now_clone = now.clone();
            web::block(move || {
                crate::db::platform::record_health_failure(&db_clone, &platform_id_clone, &now_clone, latency_ms)
            }).await.ok();
        }
        Err(e) => {
            tracing::warn!("health check ERROR: platform_id={} error={} latency={}ms", platform_id, e, latency_ms);
            let db_clone = db.clone();
            let platform_id_clone = platform_id.clone();
            let now_clone = now.clone();
            web::block(move || {
